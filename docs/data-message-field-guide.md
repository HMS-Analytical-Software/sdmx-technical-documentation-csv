...
- labels (id|name|both; default=id): This parameter applies to all Nameable SDMX Artefacts contained in the header and the body of the message:
  ...
  - If the parameter value is `both` then the concatenated id/value and localised name of the artefacts (see the section on [localised names](#localisation) on how the message deals with languages) separated by `": "` are displayed. Note that the character combination `": "` could also be part of the artefact name and could therefore occur several times within the concatenated string.
